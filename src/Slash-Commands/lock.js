        ...
        ClaimTicket.findOne({ ChannelID: interaction.channel.id }, async (err02, data02) => {
          ...
          if (data02) {
            ...
            if (data02.Locked === 'No') {
              ...
              const NotLockedLogs = new EmbedBuilder()
                .setTitle('Logs')
                .setDescription('A ticket has been locked in this guild')
                .addFields([
                  { name: 'User', value: `${interaction.user.name}`, value: true },
                  { name: 'Ticket ID', value: `${data02.TicketIDs}` }
                ])
              SupportLogs.send({ embeds: [NotLockedLogs] })
              interaction.reply('.')
              ...
              if (data02.Locked === 'Yes') {
                ...
                const NotLockedLogs = new EmbedBuilder()
                  .setTitle('Logs')
                  .setDescription('A ticket has been unlocked in this guild')
                  .addFields([
                    { name: 'User', value: `${interaction.user.name}`, value: true },
                    { name: 'Ticket ID', value: `${data02.TicketIDs}` }
                  ])
