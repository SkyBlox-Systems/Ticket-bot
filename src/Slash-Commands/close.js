...
module.exports.data = new SlashCommandBuilder()
  ...
  .addStringOption(option => 
    option.setName('id')
    .setDescription('Enter the id of the voice channel')
    .setRequired(false));
...
module.exports.run = (client, interaction) => {
  ...
  if (premiumstring === 'voice') {
    ClaimTicket.findOne({ ChannelID: idstring}, async (err, data) => {
      if (err) throw err;
      if (data) {
        MainDatabase.findOne({ ServerID: interaction.guildId}, async (err1, data1) => {
          ...
          if (data1) {
            if (data1.PaidGuild === 'Yes') {
                          ...
                          interaction.channel.awaitMessages({
                            ...
                            setTimeout(() => {
                              ClaimTicket.findOne({ ChannelID: idstring }, async (err, data) => {
                                ...
                                const voicecallchan = interaction.guild.channels.cache.get(`${data.ChannelID}`)
              ...
              } else if (data.VoiceTicket === 'Disabled') {
                interaction.reply('Voice Tickets has been disabled on this server. Please contact the owner.')
              }
            } else if (data1.PaidGuild === 'No') {
              interaction.reply('This command can only be used by people who own premium.')
            }
          } else {
            interaction.reply(`There is no voice ticket with the following ID **${idstring}**`)
          }
        })
      } else {
        interaction.reply('No stupid ticket found')
      }
    })
